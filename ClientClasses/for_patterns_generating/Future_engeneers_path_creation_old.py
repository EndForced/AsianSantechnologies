...
def show_smth(code, cords, object):
    pic = cv2.imread(f"field_pics/{code}.jpg") #создание объекта нужной картинки
...
def wave_frame_displaying(cords, w_num, max_w_num, object, toggle_ramk = 1, toggle_mycolor = None, custom_num = 0,line_smth = (3,0.8)):
    ...
    if not toggle_mycolor:
        ...
        cv2.putText(object, str(custom_num), ((cords[1] - 1) * 100 + 90 + add, cords[0] * 100 + 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                    toggle_mycolor, 2) #рисуем текст (числа после плюса подобраны вручную хд)
...
def way_visualisation(object, way, funny_sound, anim, color = (0,255,0), thickness = 10,delta = 0,field_mat = []):
    ...
    if funny_sound:
        # playsound("random_funny_stuff/what_you_get.mp3")
        print_colored("no sound hehe", "blue")
...
def final_roadmap(obj,field_mat,ramp_checkment = False, debugging = 0, min_max = 0,anim = 0, funny_sound = 0,skip_all_cv = False):
    #min_max - if != 0 displaying longest way (for fun and debugging)
    global smth_for_ramps
    smth_for_ramps = not ramp_checkment
    skip_all_cv = not skip_all_cv
        ...
        for ov in range(len(field_mat)): #клетки из которых можно разгрузить трубы
            if str(field_mat[z][ov])[0] == "6":
                ...
                if str(field_mat[z][ov])[-1] == "0" or str(field_mat[z][ov])[-1] == "1":
                    ...
                    if z+1 < len(field_mat) and str(field_mat[z+1][ov]) == "70" and (z,ov) in waves_all:
            ...
            for q in range(len(cells_to_tubes_dict[tubes[i][j]][0])): #для всех клеток с которых можно забрать конкретную трубу
                ...
                if way: #если путь есть, то записываем его в словарь с путями
                    if debugging == 1 or debugging == 0:
                        progress_bar(round(( (1/len(tubes))*i + ((1/len(tubes))/len(tubes[i])*(j+1) ) )*100))
...
def way_to_commands(path,field_mat):
    ...
    final_str = ""
    dir_list = []
    # 1-pid 0 - up 1 - down 2 - right, 3 - left, cells_num
    # 2 - grab 0 - up 1 - down 2 - right, 3 - left
    dir = 0
        ...
        for j in range(len(path[i]) - 1):
            ...
            if int(str(field_mat[path[i][j][0]][path[i][j][1]])[0]) == 3: #дядяяяяяяяяяяяяяяяя
                ...
                if int(str(field_mat[path[i][j + 1][0]][path[i][j + 1][1]])[0]) == 7:
                    dir_list.append(74) #means ramp down

                else:
                    dir_list.append(75)  # means ramp up
...
def create_path(mat,params):
    ...
    if params[-1] == 1:
        full_way = final_roadmap(obj, mat_to_change, params[1], params[0], params[2], params[4], params[3],params[-1])
        return full_way
    ...
    if params[-2] == 0:
        full_way = final_roadmap(obj,mat_to_change,params[1], params[0],params[2],params[4],params[3])
        if full_way:
...
def ini_for_nerds(mat):
    obj = cv2.imread("white_picture.jpg")  # картинка для фона
    ini(obj, mat)
    # dict = neighbour_ini(mat)
    # waves = wave_ini((7,7),dict)
    # wave_visual(waves,obj)
    # print(waves)
    # print("done")
    cv2.imshow("map", cv2.resize(obj, (600, 600)))
...
#we need to create unload path thing. becouse our def returns only path to closest unload thing. So we still have some things to do
